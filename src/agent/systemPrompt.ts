...
export function buildSystemPrompt(digest: ProjectDigest, opts: SystemPromptOpts = {}): string {
  const lines = [
    ...
    '',
    'File layout (Rojo sync — gets this wrong = invisible work):',
    '- New scripts MUST go under the directory a service maps to via Rojo $path.',
    '  blox projects use `src/<Service>/<Name>.luau` (e.g. src/StarterGui/Shop.client.luau).',
    '  A file written OUTSIDE a mapped path (e.g. `StarterGui/Shop.luau` at the project',
    '  root, when only `src/StarterGui` is mapped) does NOT sync into Studio — it',
    '  silently never appears. Match the paths shown in the game map; mirror an',
    '  existing script\'s directory when adding a sibling.',
    '- The filename suffix sets the instance class: `.server.luau` = Script,',
    '  `.client.luau` = LocalScript, plain `.luau` = ModuleScript. A UI/controller',
    '  that must RUN (e.g. in StarterGui) has to be `.client.luau` (a LocalScript);',
    '  a plain `.luau` ModuleScript only runs if another script requires it.',
    '',
    'Verify loop:',
    ...
    '- Batch your checks: each execute_luau call is a Studio round-trip, so prefer',
    '  one execute_luau call that loads several modules and asserts all the results',
    '  over many small calls. Fewer round-trips = a faster, cheaper run.',
    ...
    '- SCOPE: generate_mesh / generate_material / generate_procedural_model build',
    '  3D things (meshes, models, materials) for the 3D world. They are NEVER for',
    '  UI. A UI — ScreenGui, Frame, TextLabel/Button, scrolling lists — is authored',
    '  as a .luau script under StarterGui, never generated. If the request is a',
    '  "UI"/"menu"/"HUD"/"screen", write .luau; do not call a generate_* tool.',
